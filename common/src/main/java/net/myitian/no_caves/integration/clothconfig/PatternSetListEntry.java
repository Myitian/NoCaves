...
public class PatternSetListEntry extends BaseListEntry<Pattern, PatternSetListEntry.Cell, PatternSetListEntry> {
    ...
    public PatternSetListEntry(Text fieldName, @Nullable PatternSet value, boolean defaultExpanded, Supplier<Optional<Text[]>> tooltipSupplier, Supplier<List<Pattern>> defaultValue, Consumer<List<Pattern>> saveConsumer, Text resetButtonKey, boolean requiresRestart, boolean deleteButtonEnabled, boolean insertInFront) {
        super(
                fieldName,
                tooltipSupplier,
                defaultValue,
    ...
    public void setCellErrorSupplier(Function<Pattern, Optional<Text>> cellErrorSupplier) {
    ...
    public static class Cell extends BaseListCell {
        protected final TextFieldWidget patternFieldWidget;
        protected final TextFieldWidget flagsFieldWidget;
        ...
        public Cell(@Nullable Pattern value, PatternSetListEntry listListEntry) {
            ...
            patternFieldWidget = new TextFieldWidget(textRenderer, 0, 0, 80, 18, ScreenTexts.EMPTY) {
                @Override
            ...
            patternFieldWidget.setTooltip(Tooltip.of(Text.translatable("tooltip.no_caves.pattern.pattern")));
            patternFieldWidget.setMaxLength(999999);
            patternFieldWidget.setDrawsBackground(false);
            patternFieldWidget.setText(value == null ? "" : value.pattern());
            patternFieldWidget.setCursorToStart(false);
            patternFieldWidget.setChangedListener(s -> patternFieldWidget.setEditableColor(getPreferredTextColor()));
            flagsFieldWidget = new TextFieldWidget(textRenderer, 0, 0, 20, 18, ScreenTexts.EMPTY) {
                @Override
                public void renderWidget(DrawContext graphics, int mouseX, int mouseY, float delta) {
                    setFocused(isSelected && isFocused());
                    if (getFocused() == this && !isFocused()) {
                ...
                public void write(String text) {
                    super.write(stripFlagText(text));
                }
            };
            flagsFieldWidget.setTooltip(Tooltip.of(Text.translatable("tooltip.no_caves.pattern.flags")));
            flagsFieldWidget.setMaxLength(999999);
            flagsFieldWidget.setDrawsBackground(false);
            flagsFieldWidget.setText(value == null ? "0" : String.valueOf(value.flags()));
            flagsFieldWidget.setCursorToStart(false);
            flagsFieldWidget.setChangedListener(s -> flagsFieldWidget.setEditableColor(getPreferredTextColor()));
        }
        ...
        public void appendNarrations(NarrationMessageBuilder narrationElementOutput) {
            patternFieldWidget.appendNarrations(narrationElementOutput);
            flagsFieldWidget.appendNarrations(narrationElementOutput);
        }

        public PatternKey getValueAsPatternKey() {
            try {
                String pattern = patternFieldWidget.getText();
                String flags = flagsFieldWidget.getText();
        ...
        public Optional<Text> getError() {
            try {
                ...
                return Optional.of(Text.translatable("text.cloth-config.error.not_valid_number_int"));
            } catch (IllegalArgumentException e) {
                return Optional.of(Text.literal(NoCaves.getFirstLine(e.getLocalizedMessage())));
            }
