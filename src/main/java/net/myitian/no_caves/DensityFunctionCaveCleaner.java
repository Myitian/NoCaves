...
public final class DensityFunctionCaveCleaner {
    ...
    private static DensityFunction transformRangeChoice(DensityFunctions.RangeChoice rangeChoice) {
        ...
        } else if (input instanceof DensityFunctions.Constant(double v)) {
    ...
    private static DensityFunction transformLinear(DensityFunctions.MulOrAdd linear) {
        ...
        } else if (transformedChild instanceof DensityFunctions.Constant(double value)) {
            return DensityFunctions.constant(linear.transform(value));
        } else {
    ...
    private static DensityFunction transformBinary(DensityFunctions.TwoArgumentSimpleFunction binary) {
        ...
        if (transformedChild1 == null) {
            ...
            return transformedChild1;
        } else if (transformedChild1 instanceof DensityFunctions.Constant(double v1)
            && transformedChild2 instanceof DensityFunctions.Constant(double value)) {
            switch (binary.type()) {
                case ADD -> {
                    return DensityFunctions.constant(v1 + value);
                }
                case MUL -> {
                    return DensityFunctions.constant(v1 == 0.0 ? 0.0 : v1 * value);
                }
                case MIN -> {
                    return DensityFunctions.constant(Math.min(v1, value));
                }
                case MAX -> {
                    return DensityFunctions.constant(Math.max(v1, value));
                }
                default -> {
                }
            }
    ...
    private static DensityFunction transformUnary(DensityFunctions.PureTransformer unary) {
        ...
        if (transformedChild == null) {
            ...
            return DensityFunctions.constant(unary.transform(value));
        } else if (transformedChild != originalChild) {
